...
public class SucKhoe implements Crawler {
	@Override
	public void craw() throws IOException{
		try{
		Server.doc = Jsoup.connect("http://suckhoe.vnexpress.net/").get();
		//get all links and recursively call the processPage method
		Elements questions = Server.doc.select("a[href]");
		for(Element link: questions){
			if(link.attr("href").contains("suckhoe.vnexpress.net/")){
				...
				DBObject links = new BasicDBObject().append("Link", link.attr("abs:href"))
						 							...
						 							.append("Type","Suc Khoe")
						 							.append("Images",doc2.body().getElementsByAttributeValueContaining("src","http://img.f43.suckhoe.vnecdn.net/").attr("src"));
				BasicDBObject andQuery = new BasicDBObject();
				List<BasicDBObject> object = new ArrayList<BasicDBObject>();
				object.add(new BasicDBObject("Type", "Suc Khoe"));
			...
			System.out.println("Craw Suc Khoe news finished !");
	  }
	}
}
