...
public class GiaiTri implements Crawler {
	@Override
	public void craw() throws IOException{
		try{
		Server.doc = Jsoup.connect("http://giaitri.vnexpress.net/").get();
		//get all links and recursively call the processPage method
		Elements questions = Server.doc.select("a[href]");
		for(Element link: questions){
			if(link.attr("href").contains("giaitri.vnexpress.net")){
				...
				DBObject links = new BasicDBObject().append("Link", link.attr("abs:href"))
						 							...
						 							.append("Type","Giai Tri")
						 							.append("Images",doc2.body().getElementsByAttributeValueContaining("src","http://img.f9.giaitri.vnecdn.net").attr("src"));
				BasicDBObject andQuery = new BasicDBObject();
				List<BasicDBObject> object = new ArrayList<BasicDBObject>();
				object.add(new BasicDBObject("Type", "Giai Tri"));
			...
			System.out.println("Craw Giai Tri news finished !");
	  }
	}
}